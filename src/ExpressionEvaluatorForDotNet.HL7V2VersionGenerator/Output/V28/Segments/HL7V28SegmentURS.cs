...
public HL7V28Field URS1
{
    get
    {
        ...
        if (uRS1.field.FieldRepetitions != null && uRS1.field.FieldRepetitions.Count > 0)
        {
            ...
            for (var i = 0; i < uRS1.field.FieldRepetitions.Count; i++)
            {
                var fieldRepetition = new HL7V28FieldRepetition
                {
                    fieldRepetition = uRS1.field.FieldRepetitions[i],
                    Id = fieldData.Id,
                    Type = fieldData.Type,
                ...
                if (fieldData.FieldDatas != null && fieldData.FieldDatas.Count > 0)
                {
                    ...
                    for (var j = 0; j < uRS1.field.Components(i + 1).Count; j++)
                    {
                        var componentFieldData = fieldData.FieldDatas[j];
                        var component = new HL7V28Component
                        {
                            component = uRS1.field.FieldRepetitions[i].Components[j],
                        ...
                        if (componentFieldData.FieldDatas != null && componentFieldData.FieldDatas.Count > 0)
                        {
                            ...
                            for (var k = 0; k < uRS1.field.Components(i + 1)[j].SubComponents.Count; k++)
                            {
                                var subComponentFieldData = componentFieldData.FieldDatas[k];
                                var subComponent = new HL7V28SubComponent
                                {
                                    subComponent = uRS1.field.FieldRepetitions[i].Components[j].SubComponents[k],
            ...
            uRS1.fieldRepetitions = fieldRepetitions;
        }
